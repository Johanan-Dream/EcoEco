...
public class DcontentsController {
	...
	public ModelAndView getDonationDetail(HttpServletRequest request, int dcontents_idx) {
		...
		//d-Day 계산
		String ed=dcontents.getDcontents_endday();
	...
	public ModelAndView getContentsform(HttpServletRequest request) {
		HttpSession session=request.getSession();
		 Mem mem=(Mem)session.getAttribute("mem");
		 ModelAndView mav=new ModelAndView("client/donation/contentsform");
	...
	public ModelAndView getContentsDetail(HttpServletRequest request, int dcontents_idx) {
		...
		Dcontents dcontents=dcontentsService.select(dcontents_idx);
		dcontents.setMem(mem);
		System.out.println("dcontents??"+dcontents);
		//4단계 저장하기
